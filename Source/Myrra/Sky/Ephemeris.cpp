...
TemporalDerivatives Ephemeris::CalcTemporalDerivatives(FLOAT T, FLOAT gmsA)
{
    TemporalDerivatives D;
    D.MeanSideralTime = LIMIT_HOURS_TO_24(DEGREES_TO_HOURS(gmsA));
    D.Epsilon = obliquityAndNutationForT(T, NULL, &D.DeltaNutation);
    D.GeographicLongitudeH = DEGREES_TO_HOURS(longitudeOnEarth * longitudeOnEarthSign);
    D.ApparentSideralTimeH = D.MeanSideralTime + (D.DeltaNutation / 15 * COSD(D.Epsilon)) / 3600;
    return D;
}
...
#if !DISABLE_PLANETS
EquatorialCoordinates Ephemeris::equatorialCoordinatesForEarthsMoonAtJC(FLOAT T, TemporalDerivatives* TD, FLOAT *distance)
{
    FLOAT TSquared = T*T;
    FLOAT TCubed   = TSquared*T;
    FLOAT T4       = TCubed*T;
    ...
    // Intergrate nutation
    lambda += TD->DeltaNutation/3600;
    
    EquatorialCoordinates eqCoord = EclipticToEquatorial(lambda, beta, TD->Epsilon);
    ...
    GeocentricToTopocentric(eqCoord, TD, paralax);
    return eqCoord;
}
#endif

#if !DISABLE_PLANETS
EquatorialCoordinates  Ephemeris::equatorialCoordinatesForSunAtJC(FLOAT T, TemporalDerivatives* TD, FLOAT* distance)
{
    ...
    FLOAT eps = TD->Epsilon + 0.00256 * COSD(omega);
...
void Ephemeris::GeocentricToTopocentric(EquatorialCoordinates & eqCoord, TemporalDerivatives* TD, FLOAT paralax)
{
    // Local angle in floating degrees
    FLOAT H = (TD->ApparentSideralTimeH - TD->GeographicLongitudeH - eqCoord.ra) * 15;
...
EquatorialCoordinates  Ephemeris::equatorialCoordinatesForPlanetAtJD(SolarSystemObjectIndex solarSystemObjectIndex, JulianDay jd, FLOAT *distance)
{
    ...
    while (T != lastT)
    {
        ...
        FLOAT jd22 = (jd.day+jd.time) - TLight;
        T = T_WITH_JD(jd22,0);
