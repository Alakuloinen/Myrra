...
//всякая хуйня, производная от времени (и места на земле), можно считать отдельно для всех тел
struct TemporalDerivatives
{
    // Geographic longitude in floating hours
    FLOAT GeographicLongitudeH;

    // Apparent sideral time in floating hours
    FLOAT ApparentSideralTimeH;

    FLOAT Epsilon;

    FLOAT DeltaNutation;

    FLOAT MeanSideralTime;
};
...
public:
    friend class AMyrKingdomOfHeaven;
...
private:
    ...
    static TemporalDerivatives CalcTemporalDerivatives(FLOAT T, FLOAT Explicitgmst);
    ...
    //странно, что такой рутины тут нет, она ведь в каждой почти функции юзается
    static void GeocentricToTopocentric(EquatorialCoordinates &inout, TemporalDerivatives* TD, FLOAT paralax);
...
#if !DISABLE_PLANETS
    //моя модификация дабы избежать рутинных преобразований
    static EquatorialCoordinates equatorialCoordinatesForEarthsMoonAtJC(FLOAT JulianCentury, TemporalDerivatives* TD, FLOAT* distance);
    static EquatorialCoordinates equatorialCoordinatesForEarthsMoonAtJD(JulianDay jd, FLOAT *distance)
    {
        ...
        TemporalDerivatives TD = CalcTemporalDerivatives(T, meanGreenwichSiderealTimeAtJD(jd));
        return equatorialCoordinatesForEarthsMoonAtJC(T, &TD, distance);
    }
#endif
...
#if !DISABLE_PLANETS
     //моя модификация дабы избежать рутинных преобразований
    static EquatorialCoordinates equatorialCoordinatesForSunAtJC(FLOAT JulianCentury, TemporalDerivatives* TD, FLOAT* distance);
    static EquatorialCoordinates equatorialCoordinatesForSunAtJD(JulianDay jd, FLOAT* distance)
    {   FLOAT T = (jd.day - 2451545.0 + jd.time) / 36525;
        TemporalDerivatives TD = CalcTemporalDerivatives(T, meanGreenwichSiderealTimeAtJD(jd));
        return equatorialCoordinatesForSunAtJC(T, &TD, distance);
    }
