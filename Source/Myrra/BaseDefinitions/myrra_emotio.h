...
UENUM(BlueprintType) enum class EEmoCause : uint8
{
	...
	ObjKnown,			// объект уже знакомый,								от видимости/слышимости							из памяти эмоций
	ObjKnownComeClose,	// цель приближается,								от скорости сближения и заметности				из памяти эмоций
	ObjKnownComeAway,	// цель удаляется									от скорости удаления и заметности				из памяти эмоций
	ObjKnownClose,		// цель совсем близко								от расстояния									из памяти эмоций
	ObjKnownNear,		// цель близко, но не вплотную						от расстояния									из памяти эмоций

	ObjBigInRage,		// объект большой, источкает злобу					от его злобы и размера 							постоянное, но влияет на запоминаемую эмоцию
	...
	ObjSmallInRage,		// объект мелкий, источкает злобу					от его злобы и размера							постоянное, но влияет на запоминаемую эмоцию

	HearYouUnknown,		// услышали звуки от незнакомого существа - модулируется уровнем слышимости
	SeeYouUnknown,		// внезапно увидели незнакомое существо - модулируется уровнем видимости
	HearYouKnown,		// услышали звуки от знакомого существа - модулируется уровнем слышимости
	SeeYouKnown,		// внезапно увидели знакомое существо - модулируется уровнем видимости
