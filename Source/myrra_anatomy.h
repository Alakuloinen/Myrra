...
UENUM(BlueprintType) enum class ELDY : uint8
{
	...
	ReverseVector		UMETA(DisplayName = "/ Reverse Global Orientation Vector"),
	ReverseMyVector		UMETA(DisplayName = "/ Reverse Local Vector"),
...
ENUM_CLASS_FLAGS(ELDY); enum LDY
{
	...
	LDY_KINEMATIC = 1 << (int)ELDY::Kinematic
...
USTRUCT(BlueprintType) struct FLimb
{
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly) uint8 Clung:1;				// к опоре подвязаны констрейнтом
	...
	//является ли кинематическим
	bool IsKinematic() const {	return ((DynModel & LDY_KINEMATIC) == LDY_KINEMATIC); }
	...
	FLimb() {Clung = 0; Grabs = 0; LastlyHurt = 0; }
};
...
USTRUCT(BlueprintType) struct FWholeBodyDynamicsModel
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Audio") USoundBase* Sound;
...
USTRUCT(BlueprintType) struct FGirdle
{
	...
	//если этот пояс (одна из составляющих его частей тела) пришпилен констрейнтом к своей опоре
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly) uint8 Clung:1;
